...
import { COLUMNS_ALL_FAQS } from "../config/columnsAllFaqs";
import { allFaqsAtom } from "../recoil/atoms";

// Component
const TableAllFaqs = () => {
  // Define table data
  const tableData = useRecoilValue(allFaqsAtom);
  const tableDataLen = tableData?.length;

  // Debug
  //console.log("Debug tableAllFaqs: ", tableData);

  // Memorize data and columns to avoid re-render of data
  const columns = useMemo(() => COLUMNS_ALL_FAQS, []);
  const data = useMemo(() => {
    // Check data
    if (!tableData) {
      return [];
