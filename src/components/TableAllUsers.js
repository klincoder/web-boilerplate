...
import { COLUMNS_ALL_USERS } from "../config/columnsAllUsers";
import { allUsersAtom } from "../recoil/atoms";

// Component
const TableAllUsers = () => {
  // Define table data
  const tableData = useRecoilValue(allUsersAtom);
  const tableDataLen = tableData?.length;

  // Debug
  //console.log("Debug tableAllUsers: ", tableData);

  // Memorize data and columns to avoid re-render of data
  const columns = useMemo(() => COLUMNS_ALL_USERS, []);
  const data = useMemo(() => {
    // Check data
    if (!tableData) {
      return [];
