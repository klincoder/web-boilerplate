...
import { COLUMNS_ALL_BLOG } from "../config/columnsAllBlog";
import { allBlogAtom } from "../recoil/atoms";

// Component
const TableAllBlog = () => {
  // Define table data
  const tableData = useRecoilValue(allBlogAtom);
  const tableDataLen = tableData?.length;

  // Debug
  //console.log("Debug tableAllBlog: ", tableData);

  // Memorize data and columns to avoid re-render of data
  const columns = useMemo(() => COLUMNS_ALL_BLOG, []);
  const data = useMemo(() => {
    // Check data
    if (!tableData) {
      return [];
